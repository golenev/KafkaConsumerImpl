...
- **Когда задан `groupId` или `groupIdPrefix`.** Ветка `if (hasGroupId())` в `run()` вызывает `consumer.subscribe(listOf(topic))`. Kafka сама распределяет партиции между участниками consumer group (каждый отдельный экземпляр приложения, подключённый с тем же идентификатором группы), реагирует на ребалансы (процесс перераспределения партиций между участниками, который инициирует координатор группы при изменении состава потребителей) и требует вызова `waitForAssignment()`, чтобы дождаться фактического назначения партиций. После этого `positionToTailOrLastN` переводит смещения в нужное положение. Такой режим позволяет делить нагрузку между несколькими консюмерами и требует явного коммита смещений через `commitSync()`.
